...
const CustomFormContext = React.createContext<TCustomFormContext | undefined>(undefined);
...
>({
  ...
  return (
    <FormProvider {...props}>
      <CustomFormContext.Provider value={{ state }}>{children}</CustomFormContext.Provider>
    </FormProvider>
  );
};
...
const FormMessage = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, children, ...props }, ref) => {
    ...
    const stateError =
      state?.validationErrors && state.validationErrors?.[name] ? state.validationErrors[name] : undefined;

    const body = stateError ? stateError : error ? String(error.message) : children;
...
>(({ state, className, ...props }, ref) => {
  return (
    state?.serverError && (
      <p {...props} className={cn('text-sm font-medium text-red-500', className)} ref={ref}>
        {state.serverError}
      </p>
    )
  );
...
export { Form, FormControl, FormDescription, FormError, FormField, FormItem, FormLabel, FormMessage, useFormField };
